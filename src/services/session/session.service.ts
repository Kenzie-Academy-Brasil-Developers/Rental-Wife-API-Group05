...
export const loginService = async ({ email, password }: ILoginRequest): Promise<string> => {
    ...
    const userH = await userHRepository.findOne({ select: ["password", "id"], where: { email: email } });
